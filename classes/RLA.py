...
def entropy(proba: np.ndarray) -> float:
    ...
    proba = proba[proba != 0]
    return sum([-1 * p * math.log2(p) for p in proba])


def beta_variance(a, b):
    """
    Computes the variance of the beta function of a and b
    :param a: alpha parameter of the beta distribution
    :param b: beta parameter of the beta distribution
    :return: the varinace of the beta distribution
    """
...
class RLagent:
    ...
    def __init__(self, mdp: Env, model_type: str, gamma: float, nV: float, decision_rule: str, **kwargs):
        """
        ...
        :param kwargs: parameters regarding the decision rule, the replay and the epistemic rewards
            ...
            Regarding saving:
                format: 'full' if I aim to save everything, 'small' if I want to compress and only save the essentials
        """
        ...
        self._maxval = np.array([0,  # Qmax
                                 beta_variance(1, 1) / (1 - self._gamma),  # Ur max
        ...
        self._R[:, :, 1] = beta_variance(1, 1)  # Delta std is initialized to max
        ...
        if self._epist_rew_type not in ['diff', 'abs']:
            raise ValueError('The epistemic reward type has to be either "diff" or "abs".')
        ...
        self._format = kwargs.get('format', 'full')  # Saving format (full or small)
        if self._format not in ['full', 'small']:
            raise ValueError(f'{self._format} is not a valid format, choose from "full" or "small".')
        self._events = {}  # This will be the event memory if needed
        return
    ...
    def __entropy__(self, s: int, a: int, mode: str):
        """
    ...
    def __store_in_memory__(self, s: int, a: int, s_prime: int, rew: np.ndarray, deltaC: float, **kwargs) -> None:
        """
        ...
        :param kwargs:
            estimate: is the deltaC an estimate (back propagated from a real change) or is it the result of a real
                update. Only in the latter case will I delete a corresponding memory from the buffer, should it be
                sub-threshold.
        :return: -
        """
        ...
        if abs(deltaC) <= self._replay_thresh and self._replay_type in ['priority', 'bidir']:
            ...
            if estimate:
                # If the current value is an estimate, we don't quite trust it enough so that we'd erase an old instance
                # of this event, so we simply don't do anything
                return
            else:
                # Otherwise we need to remove a potential old copuy of this event, as now we know it is relatively
                # worthless
                to_delete = True
    ...
    def __extend_state_space__(self, s_prime) -> None:
        ...
        self._maxval[1] = beta_variance(1, 1) / (1 - self._gamma)
        ...
        self._R = np.append(self._R,
                            np.array([[[0, beta_variance(1, 1), entropy(np.ones(self._nS) / self._nS)]
                                       for _ in range(self._nA)]]), axis=0)
    ...
    def __temporal_difference_error__(self, s: int, a: int, s_prime: int, rew: np.ndarray):
        """
        ...
        self._maxval = np.nanmax(np.array([self._maxval, np.nanmax(np.nanmax(self._C, axis=1), axis=0)]), axis=0)
    ...
    def __value_iteration__(self, s: int, a: int):
        """
        ...
        """
        C_curr = self.__C_vector__(s=s, a=a, replay=True)
        C_max = np.nanmax(self._C, axis=1)  # [nS x 3] array, impossible actions (with U init to max) won't affect it
    ...
    def __find_predecessors__(self, s: int, p: float) -> None:
        ...
        for a_pred in range(self._nA):
            ...
            for s_pred in s_all:
                ...
                self.__store_in_memory__(s_pred, a_pred, s, self._R[s_pred, a_pred, :], p_pred, estimate=True)
    ...
    def __save_step__(self, virtual: bool, **kwargs) -> None:
        ...
        if len(self._events['iter']) > 0:
            if not virtual:  # If real, it's a new iteration
                it = self._events['iter'][-1] + 1
            else:  # else it's the same iteration but a new step
                it = self._events['iter'][-1]
                step = self._events['step'][-1] + 1
        self._events['iter'].append(it)
        self._events['step'].append(step)
        ...
        if s is not np.nan:
            try:
                self._events['s'].append(self._states[s])
            except AttributeError:
                self._events['s'].append(s)
        else:
            self._events['s'].append(s)
        self._events['a'].append(kwargs.get('a', np.nan))
        s_prime = kwargs.get('s_prime', np.nan)
        if s_prime is not np.nan:
            try:
                self._events['s_prime'].append(self._states[s_prime])
            except AttributeError:
                self._events['s_prime'].append(s_prime)
        else:
            self._events['s_prime'].append(s_prime)
        rew = kwargs.get('rew', [np.nan] * 3)
        self._events['r'].append(rew[0])
        self._events['hr'].append(rew[1])
        self._events['ht'].append(rew[2])
        self._events['deltaC'].append(kwargs.get('deltaC', np.nan))

        # 3) Now the Q and U values
        if self._format == 'small':
            self._events['Ur'].append(np.mean(np.nanmax(self._C[:, :, 1], axis=1)))
            self._events['Ut'].append(np.mean(np.nanmax(self._C[:, :, 2], axis=1)))
        else:
            for s_idx in range(self._nS):
                for a_idx in range(self._nA):
                    try:
                        ...
                        s = s_idx
                    self._events[f'Q_{s}_{a_idx}'].append(self._C[s_idx, a_idx, 0])
                    self._events[f'Ur_{s}_{a_idx}'].append(self._C[s_idx, a_idx, 1])
                    self._events[f'Ut_{s}_{a_idx}'].append(self._C[s_idx, a_idx, 2])
                    self._events[f'C_{s}_{a_idx}'].append(np.sum(self.__C_vector__(s=s_idx, a=a_idx)))
    ...
    def model_learning(self, s: int, a: int, s_prime: int, r: float) -> Tuple[float, float]:
        ...
        var = beta_variance(np.sum(curr_Rhist != 0), np.sum(curr_Rhist == 0))  # In the literature we must add 1
        # var = beta_variance(np.sum(curr_Rhist != 0) + 1, np.sum(curr_Rhist == 0) + 1)
        if not full:
            # If Rhist is not full *before* adding the new element, we'll only take the elements that have already been
            # filled, and we'll add one
            var = beta_variance(np.sum(curr_Rhist[0:curr_V] != 0) + 1, np.sum(curr_Rhist[0:curr_V] == 0) + 1)
        else:
        ...
        #     alpha += 1
        #     beta += 1
        if self._epist_rew_type == 'diff':
            hr = abs(var - beta_variance(alpha, beta))  # absolute change!
            ht = abs(Ht - self.__entropy__(s, a, 'trans'))
        else:
            hr = beta_variance(alpha, beta)
            ht = self.__entropy__(s, a, 'trans')
        rew = np.array([r, hr, ht])
        self._R[s, np.array(range(self._nA)) == a, :] = rew
        return hr, ht

    def inference(self, s: int, a: int, s_prime: int, rew: np.ndarray, **kwargs):
        """
    ...
    def toggle_save(self, **kwargs) -> None:
        ...
        :param kwargs:
            ...
            format: 'full' if we want to save everything, 'short' if we want condensed saving
        :return:
        """
        save_on = kwargs.get('save_on', not self._save_agent)
        if save_on:
            if self._format == 'full' and len(self._events) > 0:
                try:
                    s = self._states[-1]  # We need to make it understandable for the environment
                except AttributeError:
                    s = self._nS - 1
                if f'Q_{s}_0' not in self._events:  # If we added at least one new column
                    for s_idx in range(self._nS):
                        for a_idx in range(self._nA):
                            try:
                                s = self._states[s_idx]  # We need to make it understandable for the environment
                ...
                if self._format == 'small':
                    ...
                    try:
                        ...
                        pass
                    for s_idx in poss_states:
                        for a_idx in range(self._nA):
                            self._events[f'Q_{s_idx}_{a_idx}'] = []
                            self._events[f'Ur_{s_idx}_{a_idx}'] = []
                            self._events[f'Ut_{s_idx}_{a_idx}'] = []
                            self._events[f'C_{s_idx}_{a_idx}'] = []
    ...
    def dump_agent(self, **kwargs) -> None:
        ...
        if self._events is not None:
            ...
            events = events.astype({'iter': 'int', 'step': 'int'})
            events.to_csv(f'{path}agent{label}.csv', sep=',', index=False, encoding='utf-8')
        else:
